...
@pytest.mark.parametrize("title, content, published", [
    ...
    ("favourite pizza", "I love hawaaian", False),
...
def test_create_post_default_published_true(authorized_client, test_user, test_posts):
    res = authorized_client.post("/posts", json={"title": "stats model", "content":"content"})
    created_post = schemas.Post(**res.json())
    assert res.status_code == 201
    assert created_post.title == "stats model"
    assert created_post.content == "content"
    assert created_post.published == True
    assert created_post.owner_id == test_user['id']

def test_unauthorized_user_create_post(client,test_user, test_posts):
    res = client.post("/posts", json={"title": "stats model", "content":"content"})
    assert res.status_code == 401
